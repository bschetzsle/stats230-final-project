...
Maximizing the synthetic likelihood yields an point estimator for your parameters.

However, it is trivial to switch to a Bayesian setup, placing a prior on the parameters, and obtaining a posterior distribution of parameters instead of point estimates.
...
What if, instead of using $p(\bm{Y} | \bm{\theta})$, we use:

$$p(\bm{s_Y} | \bm{\theta})$$

where $\bm{s_Y}$ is a vector of summary statistics for $\bm{Y}$, eg. the mean, quantiles, etc.

Then, instead of targeting the posterior $p(\bm{\theta} | \bm{Y})$ in our MCMC, we can instead target $p(\bm{\theta} | \bm{s_Y})$.
...
$$\bm{s_Y} \sim \mathcal{N}(\bm{\mu(\theta)}, \bm{\Sigma(\theta)})$$
...
In otherwords, given a proposed value of $\bm{\theta}$, generate $n$ iid datasets like the observed data $\bm{Y}$.
...
With these estimates, we can evaluate the synthetic likelihood of various proposed values of $\theta$ for our observed summary statistics $\bm{s_Y}$.
